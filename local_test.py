...
l_q = 10
batch_size = 128
weights = 'data\s16_k28.pth'
data_path = 'gl_16_7.npz'
...
if __name__ == '__main__':
    ...
    vin: VIN = VIN()
    vin.load_state_dict(torch.load(weights, map_location=torch.device('cpu')))
    # Loss
    criterion = nn.CrossEntropyLoss()
    # Optimizer
    optimizer = optim.RMSprop(vin.parameters(), lr=lr, eps=1e-6)
    # Dataset transformer: torchvision.transforms
    transform = None
    # Define Dataset
    trainset = GridworldData(
        data_path, imsize=imsize, train=True, transform=transform)
